  ...
  return (
    <>
      <header className="fixed z-40 top-0 inset-x-0 flex flex-col items-center transition">
        <section
          className="Topbar relative z-20 w-full bg-primary font-bold text-xs flex justify-center h-8 px-4 transition-opacity ease-linear duration-500 opacity-100"
        >
          <div className="leading-8">Discount 10%</div>
        </section>
        <section
          className="MainNav relative z-20 wrapper-fluid-no-padding-mobile grid grid-cols-6 items-center h-16 md:h-24 transition-opacity ease-linear duration-500 opacity-100"
        >
          <nav
            className="flex lg:hidden items-center space-x-3 col-span-2 lg:col-span-1"
          >
            <button
            ...
            >
              ...
              <div className="block w-5">
                <span
                  aria-hidden="true"
                  className={`block absolute h-0.5 w-5 bg-black transform transition ease-in-out ${isHamburger ? 'rotate-45' : '-translate-y-1.5'}`}
                ></span>
                <span
                  aria-hidden="true"
                  className={`block absolute h-0.5 w-5 bg-black transform  transition ease-in-out ${isHamburger ? '-rotate-45' : 'translate-y-1'}`}
                ></span>
              </div>
            </button>
          </nav>
          <div
            className="flex justify-center lg:justify-start col-span-2 lg:col-span-1"
          >
            <Link href="/">
              <Logo />
            </Link>
          ...
          <nav
            className="hidden lg:flex items-center justify-center space-x-4 h-full lg:col-span-4"
          > 
            {
              isLoading ? (
                <div
                  className="bg-gray-100 rounded-full h-4 w-20"
                ></div>
              ) : (
                headers.map((item, i) => (
                  item.children.length > 0 ? (
                    <div key={`Menu-${i}`} className='uppercase flex font-bold h-full cursor-pointer'>
                      <div className="font-bold whitespace-nowrap h-full flex items-center justify-center group">
                        <span
                          className="after:content-[''] after:block after:w-full after:h-0.5 after:bg-black after:group-hover:scale-x-100 after:scale-x-0 after:origin-top-left after:transition after:ease-in-out after:duration-300"
                        >
                          {item.name}
                        </span>
                      </div>
                    ...
                    <div key={`Menu-${i}`} className='uppercase flex font-bold h-full cursor-pointer'>
                      <Link href={item.url} className="font-bold whitespace-nowrap h-full flex items-center justify-center group">
                        <span
                          className="after:content-[''] after:block after:w-full after:h-0.5 after:bg-black after:group-hover:scale-x-100 after:scale-x-0 after:origin-top-left after:transition after:ease-in-out after:duration-300"
                        >
                          {item.name}
          ...
          <nav
            className="flex items-center space-x-4 justify-end col-span-2 lg:col-span-1"
          >
